...
namespace Nexu_SMS.Repository
{
    public class StudentRepo:IRepositoty<Student>
    {
        ...
        public List<Student> GetStdByClass(string sem)
        {
            return contextClass.students.Where(x => x.semName == sem).ToList();
        }

        public List<Student> GetStdBySection(string stream)
        {
            return contextClass.students.Where(x => x.streamName == stream).ToList();
        }

        public List<Student> GetStdBySectionNclass(string stream, string sem)
        {
            return contextClass.students.Where(x => x.streamName == stream && x.semName == sem).ToList();
        }
    }
}
